...
def reset_downstream(plan: dict[str, Any]) -> None:
    agents = {
        agent["agent_id"]: agent
        for agent in plan["agents"]
    }

    for agent_id, _ in DOWNSTREAM:
        ...
        agent["verdict"] = None
        agent["notes"].append(
            "Reset for downstream model-review regeneration."
        )

    agents["LK-LAW"]["status"] = "READY"
...
def run_review_cycle(
    ...
    if not extraction_review.exists():
        raise FileNotFoundError(
            "Extraction model review report is required before "
            "downstream review orchestration."
        )
    ...
    for agent_id, worker in DOWNSTREAM:
    ...
    report = {
        "schema_version": "1.0",
        "runtime": "AIDPL Downstream Review Runtime",
    ...
    write_json(
        case_root / "evidence/downstream-review-runtime-report.json",
        report,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="aidpl-review-run",
        description=(
            "Regenerate all downstream Legal Kural artifacts after "
            "model-assisted extraction review."
        ),
    )
...
def main() -> int:
    ...
    print()
    print("=" * 76)
    print("LEGAL KURAL DOWNSTREAM REVIEW ORCHESTRATION")
    print("=" * 76)
