...
def reset_from_kural(plan: dict[str, Any]) -> None:
    agents = {
    ...
    for agent_id, _ in WORKERS:
        ...
        agent["verdict"] = None
        agent["notes"].append(
            "Reset after model-assisted reasoning review."
        )

    agents["LK-KURAL"]["status"] = "READY"
    plan["status"] = "ACTIVE"
...
def run_cycle(
    ...
    if not (
        case_root / "evidence/reasoning-model-review-report.json"
    ).exists():
        raise FileNotFoundError(
            "Reasoning model review report is required."
        )
    ...
    report = {
        "schema_version": "1.0",
        "runtime": "AIDPL Post-Reasoning Review Runtime",
    ...
    write_json(
        case_root
        / "evidence/post-reasoning-review-runtime-report.json",
        report,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="aidpl-review-after-reason",
        description=(
            "Regenerate artifacts after model-assisted reasoning review."
        ),
    )
...
def main() -> int:
    ...
    print()
    print("=" * 76)
    print("LEGAL KURAL POST-REASONING REVIEW ORCHESTRATION")
    print("=" * 76)
