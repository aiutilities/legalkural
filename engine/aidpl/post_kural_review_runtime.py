...
def reset_from_editor(plan: dict[str, Any]) -> None:
    agents = {
    ...
    for agent_id, _ in WORKERS:
        ...
        agent["verdict"] = None
        agent["notes"].append(
            "Reset after model-assisted Kural review."
        )

    agents["LK-EDITOR"]["status"] = "READY"
    plan["status"] = "ACTIVE"
...
def run_cycle(
    ...
    if not (
        ...
        raise FileNotFoundError(
            "Kural model review report is required."
        )
    ...
    report = {
        "schema_version": "1.0",
        "runtime": "AIDPL Post-Kural Review Runtime",
    ...
    write_json(
        case_root
        / "evidence/post-kural-review-runtime-report.json",
        report,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="aidpl-review-after-kural",
        description="Regenerate artifacts after Kural model review.",
    )
...
def main() -> int:
    ...
    print()
    print("=" * 76)
    print("LEGAL KURAL POST-KURAL REVIEW ORCHESTRATION")
    print("=" * 76)
