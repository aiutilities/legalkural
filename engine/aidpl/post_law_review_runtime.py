...
def reset_from_reason(plan: dict[str, Any]) -> None:
    agents = {
    ...
    for agent_id, _ in WORKERS:
        ...
        agent["verdict"] = None
        agent["notes"].append(
            "Reset after model-assisted law review."
        )

    agents["LK-REASON"]["status"] = "READY"
    plan["status"] = "ACTIVE"
...
def run_cycle(
    ...
    if not (
        ...
        raise FileNotFoundError(
            "Law model review report is required."
        )
    ...
    report = {
        "schema_version": "1.0",
        "runtime": "AIDPL Post-Law Review Runtime",
    ...
    write_json(
        case_root / "evidence/post-law-review-runtime-report.json",
        report,
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="aidpl-review-after-law",
        description="Regenerate artifacts after model-assisted law review.",
    )
...
def main() -> int:
    ...
    print()
    print("=" * 76)
    print("LEGAL KURAL POST-LAW REVIEW ORCHESTRATION")
    print("=" * 76)
